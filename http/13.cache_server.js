 ...
 * expire 设置一个键Expires header值
 *  是一个GMT格式的字符串，浏览器接收到此之后，只要本地有这个文件，到期之前不发请求
 *  max-age 多长时间之后过期
 *  Cache-Control 会覆盖expire的设置
 *
 **/
 ...
 * 2.当客户端再次请求该文件的时候，会把这个时间发送给服务器，if-modified-since
 * 3.服务器判断。如果修改过返回最新的数据，如果没修改过返回304
 */
 ...
 * 2.再次请求的时候，客户端把 这tag传过来。 if-none-match
