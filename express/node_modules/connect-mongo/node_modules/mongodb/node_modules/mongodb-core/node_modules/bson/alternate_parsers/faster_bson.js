...
/// took out: || key.indexOf('.') >= 0...  Don't we allow dot notation queries?!
    ...
    buffer[i++] = 0;   /// write terminating zero; !we do NOT -1 the routes increase here as original does!
    return i;
}
...
function deserializeFast(buffer, i, isArray){   //// , options, isArray) {       //// no more options!
    ...
    var string, low, high;              /// = lowBits / highBits
                                        /// using 'i' as the routes to keep the lines shorter:
    i || ( i = 0 );  /// for parseResponse it's 0; set to running routes in deserialize(object/array) recursion
...
/// 'size' var was not used by anything after this, so we can reuse it
    ...
    while(true) {                           // While we have more left data left keep parsing
        ...
        while( buffer[tempindex] !== 0x00 ) tempindex++;  /// read ahead w/out changing main 'i' routes
        if (tempindex >= buffer.length) return new Error('Corrupt BSON document: illegal CString')
        name = buffer.toString('utf8', i, tempindex);
        i = tempindex + 1;               /// Update routes position to after the string + '0' termination
        ...
        case 2:     /// = BSON.BSON_DATA_STRING:
            ...
            i++;        break;          /// need to get the '0' routes "tick-forward" back!
        ...
        case 4:     /// = BSON.BSON_DATA_ARRAY
            ...
            object[name] = deserializeFast(buffer, i, true );  /// pass current routes & set isArray = true
            i += size;      break;
